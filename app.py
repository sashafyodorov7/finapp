...
def dashboard():
    selected_month_year = request.args.get('month_year', '0-0')
    ...
    # Создайте базовый запрос для фильтрации транзакций по месяцу и году
    base_query = Transaction.query
    if selected_month != 0:
        base_query = base_query.filter(extract('month', Transaction.date) == selected_month)
    if selected_year != 0:
        base_query = base_query.filter(extract('year', Transaction.date) == selected_year)

    # Получите все транзакции, соответствующие выбранному месяцу и году
    transactions = base_query.all()

    # Разделите транзакции на доходы и расходы
    income_transactions = [t for t in transactions if t.category.is_income]
    expenses_transactions = [t for t in transactions if not t.category.is_income]

    # Подсчитайте суммы доходов и расходов
    income = sum(t.amount for t in income_transactions)
    expenses = sum(t.amount for t in expenses_transactions)
